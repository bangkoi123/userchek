from fastapi import FastAPI, HTTPException, Depends, status, File, UploadFile, BackgroundTasks, Request, Form, Header
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as response:
                ...
                print(f"  - Raw HTML sample: {html_content[:300]}")
                print(f"  - Has error message: {has_error_message}")
                print(f"  - Score: {score}/5")
                print(f"  - Indicators: {indicators}")
                print(f"  - Final status: {status}")
                print(f"  - WA Type: {wa_type}")
...
async def validate_whatsapp_deeplink_single(phone: str, identifier: str = None) -> Dict[str, Any]:
    """Enhanced deep link validation with real WhatsApp account"""
    try:
        # Get available WhatsApp account
        account = await get_whatsapp_account_for_validation(db)
        
        if not account or account.get('status') != 'active':
            print("⚠️ No active WhatsApp account available, falling back to basic deep link")
        ...
        print(f"🔗 Using real WhatsApp account: {account.get('name')} for enhanced validation")
        
        # Enhanced validation with real browser session
        result = await validate_with_real_whatsapp_account(phone, str(account['_id']), db)
        ...
        if result.get('success'):
            return {
                ...
                'details': {
                    ...
                    'provider': 'whatsapp_browser_enhanced',
                    'api_response': result.get('status', 'unknown'),
                    'confidence_score': 5 if result.get('status') == 'active' else 1,
            ...
            # Error occurred, fallback to basic deep link
            print(f"⚠️ Browser validation failed: {result.get('error')}, falling back to basic deep link")
            async with WhatsAppDeepLinkValidator() as validator:
                fallback_result = await validator.validate_single_number(phone, identifier)
                fallback_result['details']['provider'] = 'whatsapp_deeplink_fallback'
                fallback_result['details']['browser_error'] = result.get('error')
                return fallback_result
        
    except Exception as e:
        print(f"❌ Enhanced deep link validation error for {phone}: {str(e)}")
        return {
            'phone_number': phone,
            'platform': 'whatsapp',
            'status': ValidationStatus.ERROR,
...
class QuickCheckRequest(BaseModel):
    ...
    validation_method: str = 'standard'  # 'standard' or 'deeplink_profile' (for WhatsApp)
    telegram_validation_method: str = 'standard'  # 'standard', 'mtp', or 'mtp_profile' (for Telegram)
...
async def process_bulk_validation(job_id: str):
    ...
    try:
        ...
        # Get platform selection and validation method from job (default to both for backward compatibility)
        validate_whatsapp = job.get("validate_whatsapp", True)
        validate_telegram = job.get("validate_telegram", True)
        validation_method = job.get("validation_method", "standard")  # Get validation method
        ...
        if validate_whatsapp:
            ...
            if phones_to_validate:
                print(f"🚀 Starting batch validation for {len(phones_to_validate)} WhatsApp numbers using method: {validation_method}")
                
                if validation_method == 'deeplink_profile':
                    # Use Deep Link Profile validation (premium method)
                    print(f"🔗 Using Deep Link Profile validation for {len(phones_to_validate)} numbers")
                    
                    # For Deep Link Profile, we need to validate individually due to WhatsApp account requirements
                    for phone in phones_to_validate:
                        ...
                        processed_count = len(whatsapp_batch_results)
                        await db.jobs.update_one(
                            {"_id": job_id},
                            {
                                "$set": {
                    ...
                    # Use standard method - prefer CheckNumber.ai if configured, otherwise fallback
                    provider_settings = await db.admin_settings.find_one({"setting_type": "whatsapp_provider"})
...
# EMERGENCY FIX: Alternative endpoints under /api/health path to bypass proxy routing
@app.get("/api/health/admin-users")
async def get_admin_users_via_health(current_user = Depends(admin_required)):
    """Alternative endpoint for admin users via /api/health path"""
    # Get paginated users
    page = 1
    limit = 20
    skip = (page - 1) * limit
...
@app.get("/api/health/admin-stats")
async def get_admin_stats_via_health(current_user = Depends(admin_required)):
    """Alternative endpoint for admin dashboard stats via /api/health path"""
